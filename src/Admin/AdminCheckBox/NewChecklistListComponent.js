...
function NewChecklistListComponent() {
    ...
    function AddtasksFunction(e, data) {
        setGlobalData(data)
        console.log(data);
        e.stopPropagation();
        setAddTasks(true)
    ...
    return (
        <>
            {addChecklistClisk &&
                <AddChecklist
                    isModalOpen={addChecklistClisk}
                    onClickCancel={() => { setAddChecklist(false) }}
                    title={'Checklist'}
                    onSave={() => { }}
                    data={globaldata}
                />
...
export default NewChecklistListComponent
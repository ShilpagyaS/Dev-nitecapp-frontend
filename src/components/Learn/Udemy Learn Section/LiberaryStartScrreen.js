        ...
        <div className='h-full w-full'>
            ...
            <div className='h-full w-full grid grid-cols-7 mt-[10px]'>
                <div className='h-full rounded-[8px] border border-[#2F2F2F] col-span-2 p-[1px] mr-[5px] bg-[#0F0F0F]'>
                    <AccordianForPlayerSection ChapterArray={itemsArray[Counter]} onItemClicked={(content) => { currentContentFunction(`${content.content} , ${content.type} <--TYPE`) }} onRightClick={() => { if (Counter < itemsArray.length - 1) setCounter(prev => prev + 1) }} onLeftClick={() => { if (Counter > 0) setCounter(prev => prev - 1) }} />
                </div>

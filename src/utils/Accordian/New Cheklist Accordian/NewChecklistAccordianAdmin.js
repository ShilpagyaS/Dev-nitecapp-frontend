...
function NewChecklistAccordianAdmin({ title, content, type, categoryid, defaultvalue, onOpenfuncObj, tasks, onAddChecklistClick, onAddTasksCLick, onEditClick }) {
            ...
            <div className={`w-full flex border border-[#404040]  justify-between cursor-pointer items-center ${type == 'user' ? 'bg-[#040404]' : type == 'checklist' ? 'bg-[#272727]' : 'bg-[#191919] border border-transparent border-b-[#292929]'} p-[15px]`}
                ...
                {type == 'checklist' &&
                    ...
                    <div className='flex shrink-0 items-center justify-between bg-transparent w-full ' onClick={() => {
                        if (type == 'checklist')
                            router.push(`/checklist/description?id=${categoryid}`)

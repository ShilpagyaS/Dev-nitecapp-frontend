...
function DashboardLiberaryCard({ completionPercentageOuter, image, name, desc, item, isAdmin, onclickEdit }) {
    ...
    return (
        <div className='max-w-[373px] w-full h-[293px] border border-[#3C3C3C] rounded-[13px] px-[6px] py-[8px] cursor-pointer relative' onMouseEnter={() => { setishover(true) }} onMouseLeave={() => { setishover(false) }}>
            ...
            {
                isAdmin && isHover &&
                <div className="editbutton cursor-pointer flex items-center justify-center bg-primary-base p-2 rounded-full w-fit absolute top-3 right-3 " onClick={() => { onclickEdit() }}>

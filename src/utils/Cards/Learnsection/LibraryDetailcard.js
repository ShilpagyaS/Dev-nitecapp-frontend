...
function LibraryDetailcard({ isAdmin, courseName, onEditClick, onaddmoculeclick, chapter, courseId }) {
                ...
                <div className={`h-[180px] flex justify-between ${show ? 'opacity-[0.4]' : ' '}`}>
                    <div className='flex'>
                        <div className='h-[200px] w-[200px] relative rounded-[10px]'>
                            ...
                            <Image src={chapter?.image || '/asset/nodrinkinverted.webp'} fill className='rounded-[10px] object-cover' />
                        </div>
                        <div className='h-full flex flex-col ml-[30px] mt-[20px]'>
                            <p className='not-italic font-bold text-[24px] text-white'>{chapter?.name}</p>
                            <p className='not-italic font-normal text-[20px] text-[#959595]'>{`${chapter?.modules.length} Modules`}</p>
                            <p className='not-italic font-normal text-[16px] text-[#959595] mt-[25px]'>{chapter?.description}</p>
                        </div>
                    </div>
                ...
                <div className='grid grid-cols-2'>
                    {
                        chapter?.modules?.map((module) =>
                            ...
                            <Moduleinititalcard show={show} isAdmin={isAdmin} courseId={courseId} courseName={courseName} completionPercentageOuter={30} moduleInfo={module} />
                        )
                    }

...
function Customswitch({ first, setfirst }) {
    return (
        <div className='cursor-pointer relative flex ' >
            ...
            <div className={`absolute px-4 py-2 bg-transparent z-10 border-[3px] border-primary-base rounded-full ${first ? 'translate-x-[73px]' : 'translate-x-0'} h-full w-[53%] transition duration-200 ease-in-out transform `} ></div>
        </div>
    )
}
